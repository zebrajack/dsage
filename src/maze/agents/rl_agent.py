...
class RLAgentConfig:
    ...
    to be in `src/maze/agents/saved_models/{path}`. If the saved model is not
    from a PAIRED/ACCEL run, it should have the same format as in the PAIRED
    code, i.e., calling
    `checkpoint["runner_state_dict"]["agent_state_dict"]["agent"]` should return
    the agent's state dict."""
...
class RLAgent:
    ...
    codebase for PAIRED (https://github.com/ucl-dark/paired).
    ...
    def __init__(self, env_func: Callable, n_evals: int, config: RLAgentConfig):
        ...
        self.model.load_state_dict(
            checkpoint["runner_state_dict"]["agent_state_dict"]["agent"])
