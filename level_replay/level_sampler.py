...
class LevelSampler:
    ...
    def __init__(
        ...
        Inputs:
            ...
            rho: float, Minimum probability of sampling a replay level.
                Note math.round(rho * len(seeds)) will first be sampled before
                sampling replay levels.
            alpha: Smoothing factor for updating scores using an exponential
                weighted average.
        ...
        self.partial_seed_max_scores = np.ones(
            (num_actors, N), dtype=np.float) * float("-inf")
        ...
        if self.sample_full_distribution:
            ...
            }  # Buffer seeds are unique across actors
        ...
        if self.strategy.startswith("tscl"):
            ...
            self.unseen_seed_weights = np.zeros(
                N)  # Force uniform distribution over seeds
    ...
    def update_seed_score(self, actor_index, seed, score, max_score, num_steps):
        if self.sample_full_distribution and seed in self.staging_seed_set:
            ...
            score, seed_idx = self._partial_update_seed_score(actor_index,
                                                              seed,
                                                              score,
                                                              max_score,
                                                              num_steps,
    ...
    def _partial_update_seed_score(self,
                                   actor_index,
                                   seed,
                                   score,
                                   max_score,
        ...
        merged_score = partial_score + (
            score - partial_score) * num_steps / float(running_num_steps)
        merged_max_score = max(partial_max_score, max_score)

        if done:
            self.partial_seed_scores[actor_index][
                seed_idx] = 0.0  # zero partial score, partial num_steps
            ...
            total_score = (self.max_score_coef * merged_max_score +
                           (1 - self.max_score_coef) * merged_score)
            self.seed_scores[seed_idx] = (
                1 - self.alpha) * old_score + self.alpha * total_score
        else:
            self.partial_seed_scores[actor_index][seed_idx] = merged_score
            self.partial_seed_max_scores[actor_index][
                seed_idx] = merged_max_score
    ...
    def _partial_update_seed_score_buffer(self,
                                          actor_index,
                                          seed,
                                          score,
                                          num_steps,
        ...
        partial_score = self.partial_seed_scores_buffer[actor_index].get(
            seed, 0)
        partial_num_steps = self.partial_seed_steps_buffer[actor_index].get(
            seed, 0)
        ...
        if done:
            ...
            if (self.seed_scores[seed_idx] <= merged_score or
                ...
                self.seed_staleness[seed_idx] = (
                    ...
                    self.seed2timestamp_buffer[seed])
                self.working_seed_buffer_size = min(
                    self.working_seed_buffer_size + 1, self.seed_buffer_size)

                if self.track_solvable:
                    self.seed_solvable[
                        seed_idx] = self.staging_seed2solvable.get(seed, True)

            # Zero partial score, partial num_steps, remove seed from staging
            # data structures
            ...
            self.partial_seed_steps_buffer[actor_index][
                seed] = running_num_steps
    ...
    def _average_entropy(self, **kwargs):
        ...
        max_entropy = -(1.0 / num_actions) * np.log(
            1.0 / num_actions) * num_actions

        scores = -torch.exp(episode_logits) * episode_logits.sum(
            -1) / max_entropy
    ...
    def _average_grounded_signed_value_loss(self, **kwargs):
        """
        Currently assumes sparse reward s.t. reward is 0 everywhere except final
        step.
        """
        seed = kwargs["seed"]
        seed_idx = self.seed2index.get(seed, -1)
    ...
    def _one_step_td_error(self, **kwargs):
        ...
        max_t = len(rewards)
        td_errors = (rewards[:-1] + value_preds[:max_t - 1] -
                     value_preds[1:max_t]).abs()
    ...
    def _update_with_rollouts(self, rollouts, score_function):
        ...
        for actor_index in range(num_actors):
            ...
            for t in done_steps:
                ...
                # if t is 0, then this done step caused a full update of
                # previous seed last cycle
                if t == 0:
                    continue
                ...
                if self.requires_value_buffers:
                    ...
                    if self.strategy == "alt_advantage_abs":
                        score_function_kwargs[
                            "alt_returns"] = rollouts.alt_returns[start_t:t,
                                                                  actor_index]
                    ...
                    if rollouts.use_popart:
                        ...
                        score_function_kwargs[
                            "value_preds"] = rollouts.value_preds[start_t:t,
                                                                  actor_index]

                # Only perform score updates on non-cliffhanger episodes ending
                # in 'done'
                if not cliffhanger[t, actor_index]:
                    ...
                    if self.grounded_values is not None:
                        seed_idx = self.seed2index.get(
                            seed_t, -1)  # Will be > -1 if seed in working seeds
                        score_function_kwargs["seed_idx"] = seed_idx
                        grounded_value_ = rollouts.rewards[start_t:t].sum(
                            0)[actor_index]
                        if seed_idx != -1:
                            grounded_value = max(self.grounded_values[seed_idx],
                                                 grounded_value_)
                        else:
                    ...
                    _, seed_idx = self.update_seed_score(
                        actor_index, seed_t, score, max_score, num_steps)
            ...
            if start_t < total_steps:
                ...
                if self.requires_value_buffers:
                    ...
                    if self.strategy == "alt_advantage_abs":
                        score_function_kwargs[
                            "alt_returns"] = rollouts.alt_returns[start_t:,
                                                                  actor_index]
                    score_function_kwargs["rewards"] = rollouts.rewards[
                        start_t:, actor_index]
                    ...
                    if rollouts.use_popart:
                        ...
                        score_function_kwargs[
                            "value_preds"] = rollouts.value_preds[start_t:,
                                                                  actor_index]
                ...
                if self.sample_full_distribution and seed_t in self.staging_seed_set:
                    self._partial_update_seed_score_buffer(
                        actor_index, seed_t, score, num_steps)
                else:
                    self._partial_update_seed_score(actor_index, seed_t, score,
                                                    max_score, num_steps)
    ...
    def after_update(self):
        ...
        # Reset partial updates, since weights have changed, and thus logits are
        # now stale
        for actor_index in range(self.partial_seed_scores.shape[0]):
            for seed_idx in range(self.partial_seed_scores.shape[1]):
                if self.partial_seed_scores[actor_index][seed_idx] != 0:
                    self.update_seed_score(actor_index, self.seeds[seed_idx], 0,
                                           float("-inf"), 0)
            ...
            for actor_index in range(self.num_actors):
                ...
                for seed in actor_staging_seeds:
                    if self.partial_seed_scores_buffer[actor_index][seed] > 0:
                        self.update_seed_score(actor_index, seed, 0,
                                               float("-inf"), 0)
        ...
        if self.sample_full_distribution:
            ...
            if self.seed_buffer_size > 0:
                if self.replay_schedule == "fixed":
                    if (proportion_filled >= self.rho and
                ...
                # If seed buffer has length 0, then just sample new random seed
                # each time
                return False
            ...
            if proportion_seen >= self.rho:
                # Sample replay level with fixed replay_prob OR if all levels
                # seen
                if np.random.rand(
                ) < self.replay_prob or not proportion_seen < 1.0:
                    return True
            ...
            proportion_seen = self._proportion_filled
            if proportion_seen >= self.rho and np.random.rand(
            ) < proportion_seen:
                return True
            else:
                return False
    ...
    def observe_external_unseen_sample(self, seeds, solvable=None):
        for i, seed in enumerate(seeds):
            ...
            if not (seed in self.staging_seed_set or
                    seed in self.working_seed_set):
    ...
    def _sample_replay_level(self, update_staleness=True):
        ...
        if np.isclose(np.sum(sample_weights), 0):
            sample_weights = np.ones_like(self.seeds, dtype=np.float) / len(
                self.seeds)
        ...
        seed_idx = np.random.choice(range(len(self.seeds)), 1,
                                    p=sample_weights)[0]
    ...
    def sample(self, strategy=None):
        ...
        if not self.sample_full_distribution:
            ...
            if strategy == "sequential":
                ...
                self.next_seed_index = (self.next_seed_index + 1) % len(
                    self.seeds)
                seed = self.seeds[seed_idx]
                return int(seed)
    ...
    def sample_weights(self):
        weights = self._score_transform(self.score_transform, self.temperature,
                                        self.seed_scores)
        ...
        if self.staleness_coef > 0:
            staleness_weights = self._score_transform(
                self.staleness_transform, self.staleness_temperature,
                self.seed_staleness)
            staleness_weights = staleness_weights * (1 -
                                                     self.unseen_seed_weights)
            z = np.sum(staleness_weights)
            if z > 0:
                staleness_weights /= z
            else:
                staleness_weights = (1.0 / len(staleness_weights) *
                                     (1 - self.unseen_seed_weights))

            weights = (1 - self.staleness_coef
    ...
    def _score_transform(self, transform, temperature, scores):
        ...
        if transform == "max":
            ...
            scores[self.unseen_seed_weights > 0] = -float(
                "inf")  # only argmax over seen levels
            argmax = np.random.choice(
                np.flatnonzero(np.isclose(scores, scores.max())))
            weights[argmax] = 1.0
